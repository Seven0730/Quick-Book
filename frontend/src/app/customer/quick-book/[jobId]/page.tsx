...
export default function CustomerQuickBookWaitingPage() {
  ...
  // -- derive remaining seconds from job.createdAt --
  ...
  // -- when timer hits zero, force a refetch so status flips to DESTROYED --
  useEffect(() => {
    if (remaining === 0) {
      refetch();
    }
  ...
  // -- polling every 5s so we also pick up BOOKED quickly --
  useEffect(() => {
